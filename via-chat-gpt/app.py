...
def render_bug_ai_output(json_file_code, json_file_path):
    ...
    tooltips = []
    file_code_strip = [s.strip() for s in json_file_code]
    for line, element in issue_map.items():
        if(line in file_code_strip):
            index = file_code_strip.index(line.strip())  # Get the corresponding line index
        ...
        children=[
            ...
            html.Div(
                ...
                style={
                    ...
                    "overflowY": "auto",
                    "fontSize": "14px",
                    "width": "60%",  # Shortened width for better tooltip visibility
...
app.layout = dbc.Container([
    ...
    # File upload section (independent of tabs)
    dbc.Row([
        dbc.Col([
            dcc.Upload(
                id="upload-file",
            ...
            html.A("Download Generated File", id="download-link", href="", target="_blank",
                   className="btn btn-secondary mt-3", style={"display": "none"})
        ], width=12),
    ], className="mb-4"),
    # Tabs section
    dbc.Tabs([
